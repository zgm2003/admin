...
func (r updateRequest) input() (user.PersonalProfileInput, error) {
	if r.Username == "" {
		return user.PersonalProfileInput{}, apperror.InvalidRequest(fmt.Errorf("username is required"))
	}
	var birthday *time.Time
	if r.Birthday != nil {
		parsed, err := time.Parse("2006-01-02", *r.Birthday)
		if err != nil {
			return user.PersonalProfileInput{}, apperror.InvalidRequest(fmt.Errorf("birthday is invalid"))
		}
		birthday = &parsed
	}
	return user.PersonalProfileInput{Username: r.Username, Phone: r.Phone, Birthday: birthday, Gender: r.Gender}, nil
}
