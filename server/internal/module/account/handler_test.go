...
type profileServiceStub struct {
	current user.PersonalProfile
	updated user.PersonalProfile
	actor   int64
	target  int64
	input   user.PersonalProfileInput
}

func (s *profileServiceStub) CurrentProfile(context.Context, int64) (user.PersonalProfile, error) {
	return s.current, nil
}

func (s *profileServiceStub) UpdatePersonalProfile(_ context.Context, actor, target int64, input user.PersonalProfileInput) (user.PersonalProfile, error) {
...
func TestProfileRoutesReadAndUpdateCurrentAdmin(t *testing.T) {
	...
	profile := &profileServiceStub{
		current: user.PersonalProfile{Current: user.Current{ID: 7, Username: "alice", Email: "alice@example.com", Phone: nil}},
		updated: user.PersonalProfile{Current: user.Current{ID: 7, Username: "alice-new", Email: "alice@example.com", Phone: nil}, UpdatedAt: time.Date(2026, 8, 28, 0, 0, 0, 0, time.UTC)},
	}
