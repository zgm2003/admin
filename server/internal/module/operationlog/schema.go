...
func PrepareSchema(ctx context.Context, db *gorm.DB) error {
	...
	if err := db.Raw(`SELECT to_regclass(current_schema() || '.sys_operation_log') IS NOT NULL`).Scan(&tableExists).Error; err != nil {
	...
	if err := db.Transaction(func(tx *gorm.DB) error {
		for _, statement := range []string{
			`ALTER TABLE sys_operation_log ADD COLUMN IF NOT EXISTS event_id VARCHAR(64)`,
			`UPDATE sys_operation_log SET event_id = 'legacy-' || id::text WHERE event_id IS NULL OR event_id = ''`,
			`ALTER TABLE sys_operation_log ALTER COLUMN event_id SET NOT NULL`,
		} {
...
func EnsureSchema(ctx context.Context, db *gorm.DB) error {
	...
	indexes := []string{
		"DROP INDEX IF EXISTS ux_sys_operation_log_request_id",
		"CREATE UNIQUE INDEX IF NOT EXISTS ux_sys_operation_log_event_id ON sys_operation_log (event_id)",
		"CREATE INDEX IF NOT EXISTS ix_sys_operation_log_request_id ON sys_operation_log (request_id)",
