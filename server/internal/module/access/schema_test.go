...
func TestAccessVersionSchemaBackfillsUsers(t *testing.T) {
	...
	createdUser := user.User{Username: fmt.Sprintf("version-%d", now.UnixNano()), Email: fmt.Sprintf("version-%d@example.com", now.UnixNano()), PasswordHash: "hash", IsEnabled: yesno.Yes, CreatedAt: now, UpdatedAt: now}
