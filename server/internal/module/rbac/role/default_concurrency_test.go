...
func TestSetDefaultSerializesConcurrentWriters(t *testing.T) {
	...
	var count int64
	if err := db.WithContext(ctx).Raw("SELECT COUNT(*) FROM rbac_role WHERE is_default = ? AND deleted_at IS NULL", yesno.Yes).Scan(&count).Error; err != nil {
		t.Fatal(err)
	}
	if count != 1 {
	...
	var enabled int64
	if err := db.WithContext(ctx).Raw("SELECT COUNT(*) FROM rbac_role WHERE is_default = ? AND is_enabled = ? AND deleted_at IS NULL", yesno.Yes, yesno.Yes).Scan(&enabled).Error; err != nil {
		t.Fatal(err)
	}
	if enabled != 1 {
