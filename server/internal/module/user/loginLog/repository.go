...
func (r *Repository) Insert(ctx context.Context, event Event) error {
	...
	now := time.Now().UTC()
	value := LoginLog{UserID: event.UserID, PlatformID: event.PlatformID, Account: event.Account, EventType: event.EventType, LoginType: event.LoginType, IsSuccess: event.IsSuccess, ReasonCode: event.ReasonCode, ClientIP: event.ClientIP, UserAgent: event.UserAgent, CreatedAt: now, UpdatedAt: now}
...
func (r *Repository) List(ctx context.Context, query ListQuery) ([]Item, int64, error) {
	...
	if query.Account != "" {
		db = db.Where("login_log.account ILIKE ?", query.Account+"%")
	}
	if query.From != nil {
	...
	if err := db.Select("login_log.id, login_log.user_id, COALESCE(platform.code, '') AS platform, login_log.account, login_log.event_type, login_log.login_type, login_log.is_success, login_log.reason_code, login_log.client_ip, login_log.user_agent, login_log.created_at").
