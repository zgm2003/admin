...
func TestHandlerUserSuccessContracts(t *testing.T) {
	...
	service := &userHTTPService{
		listResult:  pagination.Result[user.ListItem]{List: []user.ListItem{{ID: 7, Username: "alice", Email: "alice@example.com", Phone: &phone, IsEnabled: yesno.Yes, Roles: []user.RoleSummary{{ID: 2, Code: "member", Name: "Member", IsEnabled: yesno.Yes}}, CreatedAt: now, UpdatedAt: now}}, Total: 1, Page: 1, PageSize: 20},
		roleOptions: []user.RoleSummary{},
		updated:     user.UpdatedProfile{ID: 7, Username: "alice_new", Phone: &phone, UpdatedAt: now},
		roles:       user.Roles{User: user.Summary{ID: 7, Username: "alice", Email: "alice@example.com", Phone: &phone, IsEnabled: yesno.Yes}, Roles: []user.RoleSummary{}, RoleIDs: []int64{}},
		roleCount:   2,
	}
	...
	if service.listQuery != (user.ListQuery{Page: 1, PageSize: 20}) || service.actorID != 41 || service.targetID != 7 || service.updateInput.Username != "alice_new" || service.updateInput.Phone == nil || *service.updateInput.Phone != phone || service.statusValue != yesno.No || !reflect.DeepEqual(service.roleIDs, []int64{5, 2, 5}) {
...
func TestHandlerUserUpdateRequiresNullablePhoneAndReturnsClosedProfiles(t *testing.T) {
	now := time.Date(2026, 8, 20, 1, 2, 3, 4, time.UTC)
	service := &userHTTPService{
		listResult: pagination.Result[user.ListItem]{List: []user.ListItem{{ID: 7, Username: "alice", Email: "alice@example.com", IsEnabled: yesno.Yes, Roles: []user.RoleSummary{}, CreatedAt: now, UpdatedAt: now}}, Total: 1, Page: 1, PageSize: 20},
		updated:    user.UpdatedProfile{ID: 7, Username: "alice", UpdatedAt: now},
...
func TestHandlerUserUpdateLogsProfileOperationWithoutPhone(t *testing.T) {
	...
	now := time.Date(2026, 8, 20, 1, 2, 3, 4, time.UTC)
	service := &userHTTPService{updated: user.UpdatedProfile{ID: 7, Username: "alice", Phone: pointerToUserHandler(phone), UpdatedAt: now}}
	...
	user.RegisterRoutes(router.Group("/api/admin/v1"), user.NewHandler(service, func(*gin.Context) (int64, bool) { return 41, true }), pass, func(string) gin.HandlerFunc { return pass })
...
func (s *userHTTPService) Update(_ context.Context, actor, target int64, input user.UpdateInput) (user.UpdatedProfile, error) {
	s.calls++
	s.actorID, s.targetID, s.updateInput = actor, target, input
	if _, err := user.NormalizePhone(input.Phone); err != nil {
		return user.UpdatedProfile{}, apperror.InvalidRequest(err)
	}
