...
func (c *AuthorityCoordinator) Mutate(ctx context.Context, current Current, mutation func(context.Context) error) error {
	if c == nil || c.states == nil || c.invalidator == nil || current.UserID < 1 || !current.IsEnabled || current.Deleted {
		return fmt.Errorf("phone authority mutation dependencies are unavailable")
	}
	...
	if err := lease.Commit(cleanupCtx, authstate.MutationFacts{Users: []authstate.UserFact{{
		UserID: current.UserID, Generation: nextGeneration, IsEnabled: current.IsEnabled, Deleted: current.Deleted,
	}}}); err != nil {
		return errors.Join(renewalErr, err)
	}
	return nil
