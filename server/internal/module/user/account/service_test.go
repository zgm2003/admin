...
func TestListServiceValidatesQueryAndReturnsNonNilEmptyPage(t *testing.T) {
	...
	for _, query := range []account.ListQuery{
		{Page: 0, PageSize: 20},
		{Page: 1, PageSize: 0},
		{Page: 1, PageSize: 101},
...
func TestRoleOptionsServiceReturnsRepositoryOptions(t *testing.T) {
	tx, ctx, _ := openUserTransaction(t)
	options, err := newUserTestService(t, account.NewRepository(tx)).RoleOptions(ctx)
...
func TestServiceUpdateProfileNormalizesPhoneAndPreservesNoOpTimestamp(t *testing.T) {
	...
	updated, err := service.Update(ctx, created.ID, created.ID, account.UpdateInput{Username: "  新用户名_01  ", Phone: &phone})
	...
	idempotent, err := service.Update(ctx, created.ID, created.ID, account.UpdateInput{Username: "新用户名_01", Phone: updated.Phone})
...
func TestServiceUpdateProfileSetsPhoneWhenUsernameIsUnchanged(t *testing.T) {
	...
	phone := "+86 138-0000-0000"
	updated, err := newUserTestService(t, account.NewRepository(tx)).Update(ctx, created.ID, created.ID, account.UpdateInput{Username: created.Username, Phone: &phone})
...
func TestServiceUpdateProfileClearsPhoneWhenUsernameIsUnchanged(t *testing.T) {
	...
	updated, err := newUserTestService(t, account.NewRepository(tx)).Update(ctx, created.ID, created.ID, account.UpdateInput{Username: created.Username, Phone: nil})
...
func TestServiceUpdateProfileValidatesTargetAndConflicts(t *testing.T) {
	...
	for _, name := range []string{"ab", "bad name", strings.Repeat("a", 65)} {
		if _, err := service.Update(ctx, actor.ID, target.ID, account.UpdateInput{Username: name, Phone: nil}); appErrorCodeForUser(err) != apperror.CodeInvalidRequest {
			t.Errorf("Update(%q) error = %v", name, err)
		}
	}
	for _, phone := range []string{"", "  ", "123\n456", strings.Repeat("1", 33)} {
		if _, err := service.Update(ctx, actor.ID, target.ID, account.UpdateInput{Username: target.Username, Phone: &phone}); appErrorCodeForUser(err) != apperror.CodeInvalidRequest {
			t.Errorf("Update(phone=%q) error = %v", phone, err)
		}
	}
	if _, err := service.Update(ctx, actor.ID, 9223372036854770000, account.UpdateInput{Username: "valid_name", Phone: nil}); appErrorCodeForUser(err) != account.CodeUserNotFound {
		t.Fatalf("unknown target error = %v", err)
	}
	if _, err := service.Update(ctx, actor.ID, target.ID, account.UpdateInput{Username: strings.ToUpper(actor.Username), Phone: nil}); appErrorCodeForUser(err) != account.CodeUserUsernameConflict {
		t.Fatalf("conflict error = %v", err)
	}
	phone := "+86 138-0000-0000"
	if _, err := service.Update(ctx, actor.ID, actor.ID, account.UpdateInput{Username: actor.Username, Phone: &phone}); err != nil {
		t.Fatal(err)
	}
	if _, err := service.Update(ctx, target.ID, target.ID, account.UpdateInput{Username: target.Username, Phone: &phone}); appErrorCodeForUser(err) != account.CodeUserPhoneConflict {
		t.Fatalf("phone conflict error = %v", err)
	}
	if err := tx.WithContext(ctx).Delete(&target).Error; err != nil {
		t.Fatal(err)
	}
	if _, err := service.Update(ctx, actor.ID, target.ID, account.UpdateInput{Username: "deleted_target", Phone: nil}); appErrorCodeForUser(err) != account.CodeUserNotFound {
		t.Fatalf("deleted target error = %v", err)
	}
	if _, err := service.Update(ctx, actor.ID, actor.ID, account.UpdateInput{Username: target.Username, Phone: nil}); err != nil {
...
func TestServiceUpdateProfileProtectsSuperAdminTarget(t *testing.T) {
	...
	service := newUserTestService(t, account.NewRepository(tx))
	if _, err := service.Update(ctx, ordinary.ID, superTarget.ID, account.UpdateInput{Username: "blocked_super", Phone: nil}); appErrorCodeForUser(err) != account.CodeUserSuperAdminProtected {
		t.Fatalf("ordinary actor error = %v", err)
	}
	updated, err := service.Update(ctx, superActor.ID, superTarget.ID, account.UpdateInput{Username: "allowed_super", Phone: nil})
...
func TestServiceUpdateProfileMapsRepositoryFailure(t *testing.T) {
	...
	cancel()
	_, err = newUserTestService(t, account.NewRepository(tx)).Update(canceled, actor.ID, actor.ID, account.UpdateInput{Username: "failure_name", Phone: nil})
...
func TestServiceUpdateProfileRollsBackUsernameAndPhoneOnWriteFailure(t *testing.T) {
	...
	if err := tx.WithContext(ctx).Model(&account.User{}).Where("id = ?", target.ID).Update("phone", &oldPhone).Error; err != nil {
		t.Fatal(err)
	}
	...
	if _, err := newUserTestService(t, account.NewRepository(tx)).Update(ctx, target.ID, target.ID, account.UpdateInput{Username: "rollback_new", Phone: &newPhone}); appErrorCodeForUser(err) != apperror.CodeDependencyUnavailable {
...
func TestServiceUpdateRolesNormalizesDiffAndPreservesHistory(t *testing.T) {
	...
	var firstUpdated account.User
	if err := tx.WithContext(ctx).Take(&firstUpdated, target.ID).Error; err != nil {
		t.Fatal(err)
	}
	relations, err := account.NewRepository(tx).FindUserRoles(ctx, target.ID)
	...
	if _, err := service.UpdateRoles(ctx, actor.ID, target.ID, []int64{disabled.ID}); appErrorCodeForUser(err) != account.CodeUserInvalidRoles {
		t.Fatalf("disabled-only error = %v", err)
	}
	if _, err := service.UpdateRoles(ctx, actor.ID, target.ID, []int64{}); appErrorCodeForUser(err) != account.CodeUserInvalidRoles {
		t.Fatalf("empty error = %v", err)
	}
	if _, err := service.UpdateRoles(ctx, actor.ID, target.ID, []int64{defaultRole.ID, 9223372036854770000}); appErrorCodeForUser(err) != account.CodeUserRoleNotFound {
		t.Fatalf("missing role error = %v", err)
	}
	if _, err := service.UpdateRoles(ctx, target.ID, target.ID, []int64{defaultRole.ID}); appErrorCodeForUser(err) != account.CodeUserSelfOperation {
...
func TestServiceUpdateRolesProtectsSuperAdministratorAssignments(t *testing.T) {
	...
	service := newUserTestService(t, account.NewRepository(tx))
	if _, err := service.UpdateRoles(ctx, ordinary.ID, superTarget.ID, []int64{defaultRole.ID}); appErrorCodeForUser(err) != account.CodeUserSuperAdminProtected {
		t.Fatalf("ordinary super-target error = %v", err)
	}
	if _, err := service.UpdateRoles(ctx, ordinary.ID, normalTarget.ID, []int64{defaultRole.ID, superRole.ID}); appErrorCodeForUser(err) != account.CodeUserSuperAdminProtected {
...
func TestServiceUpdateRolesRedisFailurePreventsPostgreSQLMutation(t *testing.T) {
	...
	service, _, _, redisClient := newUserMutationTestService(t, account.NewRepository(tx))
...
func TestServiceConcurrentSuperAdminRemovalsCannotBothCommit(t *testing.T) {
	...
	for updateErr := range errorsByOperation {
		...
		if appErrorCodeForUser(updateErr) == account.CodeUserSuperAdminProtected {
			failures++
			continue
		}
...
func TestServiceUpdateStatusDisablesRevokesAndPublishesFreshStates(t *testing.T) {
	...
	service, authStates, accessStates, _ := newUserMutationTestService(t, account.NewRepository(tx))
	beforeVersion := readUserAccessVersion(t, tx, ctx, target.ID)
	...
	if err := service.UpdateStatus(ctx, actor.ID, actor.ID, yesno.No); appErrorCodeForUser(err) != account.CodeUserSelfOperation {
...
func TestServiceUpdateStatusProtectsSuperAdminTarget(t *testing.T) {
	...
	if err := newUserTestService(t, account.NewRepository(tx)).UpdateStatus(ctx, ordinary.ID, superTarget.ID, yesno.No); appErrorCodeForUser(err) != account.CodeUserSuperAdminProtected {
...
func TestServiceDeleteSoftDeletesRelationsRevokesSessionsAndPublishesStates(t *testing.T) {
	...
	if err := service.Delete(ctx, actor.ID, 9223372036854770000); appErrorCodeForUser(err) != account.CodeUserNotFound {
		t.Fatalf("unknown delete error = %v", err)
	}
	if err := service.Delete(ctx, actor.ID, actor.ID); appErrorCodeForUser(err) != account.CodeUserSelfOperation {
...
func newUserTestService(t *testing.T, repository *account.Repository) *account.Service {
	t.Helper()
