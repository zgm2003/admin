...
// writing so a rejected request never debits either window.
...
func (l *RedisLimiter) Reserve(ctx context.Context, requests ...LimitRequest) (LimitResult, error) {
	...
	for _, request := range requests {
		if request.Key == "" || request.Limit < 1 || request.Limit > 100000 || request.Window <= 0 || request.Window.Seconds() > 86400 {
			return LimitResult{}, fmt.Errorf("invalid mail rate limit request")
		}
		if request.Key == "" || seen[request.Key] {
