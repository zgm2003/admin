...
func RegisterRoutes(group *gin.RouterGroup, handler *Handler, authenticate gin.HandlerFunc, requirePermission func(string) gin.HandlerFunc) {
	...
	group.DELETE("/log/:id", authenticate, requirePermission(PermissionDelete), handler.Delete)
	group.DELETE("/log", authenticate, requirePermission(PermissionDelete), handler.DeleteMany)
}
