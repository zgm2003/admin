	...
	for range 2 {
		...
		go func() {
			...
			value, err := repository.CreatePending(ctx, &Model{
				...
				ToEmail: "user@example.com", Subject: "subject", Status: StatusPending, CreatedAt: now, UpdatedAt: now,
			})
...
VALUES (7, 'tester', 'user@example.com', '', 1, ?, ?);`, now, now).Error; err != nil {
	...
	adminLog, err := repository.CreatePending(ctx, &Model{
		...
		Subject: "subject", Status: StatusSent, CreatedAt: now, UpdatedAt: now,
	})
	if err != nil {
		t.Fatal(err)
	}
	canvasLog, err := repository.CreatePending(ctx, &Model{
		...
		Subject: "subject", Status: StatusFailed, CreatedAt: now, UpdatedAt: now,
	})
	if err != nil {
		t.Fatal(err)
	}
	older, err := repository.CreatePending(ctx, &Model{
		...
		Subject: "subject", Status: StatusSent, CreatedAt: now.Add(-48 * time.Hour), UpdatedAt: now,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	statusRows, total, err := repository.List(ctx, ListQuery{Status: StatusFailed}, 1, 20)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRepositoryFindCrossPlatform(t *testing.T) {
	database, ctx := openRepositoryDatabase(t)
	repository := NewRepository(database)
	now := time.Now().UTC().Truncate(time.Microsecond)
	if err := database.WithContext(ctx).Exec(`
...
func TestRepositoryEscapesPrefixWildcards(t *testing.T) {
	database, ctx := openRepositoryDatabase(t)
	repository := NewRepository(database)
	now := time.Now().UTC().Truncate(time.Microsecond)
	for id, email := range map[int64]string{1: "literal%value@example.com", 2: "literal_value@example.com", 3: `literal\value@example.com`} {
		if _, err := repository.CreatePending(ctx, &Model{
			ID: id, PlatformID: 1, Scene: "login", TemplateID: 47941, ToEmail: email,
			Subject: "subject", Status: StatusSent, CreatedAt: now, UpdatedAt: now,
		}); err != nil {
			t.Fatal(err)
		}
	...
	for _, test := range []struct {
		...
		{prefix: "literal%", id: 1},
		{prefix: "literal_", id: 2},
		{prefix: `literal\`, id: 3},
	} {
		rows, total, err := repository.List(ctx, ListQuery{ToEmail: test.prefix}, 1, 20)
		if err != nil {
			t.Fatal(err)
		}
		if total != 1 || len(rows) != 1 || rows[0].ID != test.id {
			t.Fatalf("prefix %q returned total=%d rows=%+v", test.prefix, total, rows)
		}
	}
}
...
CREATE TABLE message_mail_log(
 ...
 to_email varchar(254) NOT NULL, subject varchar(255) NOT NULL, status smallint NOT NULL,
...
CREATE TABLE message_mail_log_verification(
 ...
 mail_log_id bigint NOT NULL,key_version varchar(16) NOT NULL,code_ciphertext text NOT NULL,
	...
	CREATE UNIQUE INDEX ux_message_mail_verification_log
