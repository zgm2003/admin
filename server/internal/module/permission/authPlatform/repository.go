...
func (r *Repository) FindSessionLimitCandidates(ctx context.Context, platform string, maxSessions int16) ([]int64, error) {
	...
	if err := r.db.WithContext(ctx).Raw(`
		...
		JOIN permission_auth_platform AS platform_ref ON platform_ref.id = session.platform_id
...
func (r *Repository) LockActiveSessionUsers(ctx context.Context, platform string) ([]int64, error) {
	...
	if err := r.db.WithContext(ctx).Raw(`
		...
		WHERE app_user.id IN (
			SELECT session.user_id FROM user_session AS session JOIN permission_auth_platform AS platform_ref ON platform_ref.id = session.platform_id WHERE platform_ref.code = ? AND session.revoked_at IS NULL
		)
...
func (r *Repository) RevokePlatformSessions(ctx context.Context, platform string, now time.Time) ([]SessionRef, error) {
	...
	if err := r.db.WithContext(ctx).Table("user_session AS session").Joins("JOIN permission_auth_platform AS platform_ref ON platform_ref.id = session.platform_id").Clauses(clause.Locking{Strength: "UPDATE"}).Where("platform_ref.code = ? AND session.revoked_at IS NULL", platform).
...
func (r *Repository) EnforcePlatformLimit(ctx context.Context, platform string, maxSessions int16, now time.Time) ([]SessionRef, error) {
	...
	if err := r.db.WithContext(ctx).Clauses(clause.Locking{Strength: "UPDATE"}).
		Table("user_session AS session").Joins("JOIN permission_auth_platform AS platform_ref ON platform_ref.id = session.platform_id").Where("platform_ref.code = ? AND session.revoked_at IS NULL", platform).
		Order("user_id ASC, created_at DESC, id DESC").Find(&rows).Error; err != nil {
...
func mapWriteError(operation string, err error) error {
	...
	if errors.As(err, &postgresError) && postgresError.Code == "23505" && postgresError.ConstraintName == "ux_permission_auth_platform_code_active" {
