...
func preparePlatformSessionSchema(t *testing.T, db *gorm.DB, ctx context.Context) {
	t.Helper()
	if err := database.AutoMigrate(ctx, db, &user.User{}, &auth.Session{}, &authplatform.Platform{}, &menu.Menu{}, &ratelimitpolicy.Model{}, &smsratelimitpolicy.Model{}); err != nil {
		t.Fatal(err)
	}
...
INSERT INTO system_config_cache_generation(namespace, scope_key, generation) VALUES
 ('message.mail', 'global', 1), ('message.sms', 'global', 1);`).Error; err != nil {
		t.Fatal(err)
	}
...
func newPlatformLifecycleRepository(db *gorm.DB) *authplatform.Repository {
	...
	mailScope := cachegeneration.Scope{Namespace: "message.mail", ScopeKey: "global"}
	smsScope := cachegeneration.Scope{Namespace: "message.sms", ScopeKey: "global"}
	repository.SetRateLimitPolicyLifecycle(
		func(ctx context.Context, tx *gorm.DB, platformID int64, expected authplatform.RateLimitGenerationBases, now time.Time) (authplatform.RateLimitGenerationEvents, error) {
			...
			smsEvent, err := generations.AdvanceTx(ctx, tx, smsScope, expected.SMS, now)
			if err != nil {
				return authplatform.RateLimitGenerationEvents{}, err
			}
...
func assertActivePlatformSessionIDs(t *testing.T, db *gorm.DB, ctx context.Context, platform string, want []int64) {
	t.Helper()
	got := make([]int64, 0)
	if err := db.WithContext(ctx).Table("user_session AS session").Joins("JOIN permission_auth_platform AS platform_ref ON platform_ref.id = session.platform_id").Where("platform_ref.code = ? AND session.revoked_at IS NULL", platform).Order("session.id ASC").Pluck("session.id", &got).Error; err != nil {
		t.Fatal(err)
	}
