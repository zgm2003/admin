...
func TestManagementListRequiresStrictQueryAndReturnsClosedPage(t *testing.T) {
	...
	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodGet, "/api/admin/v1/permission/authplatform?page=1&pageSize=20&keyword=admin&isEnabled=1", nil)
	router.ServeHTTP(recorder, request)
	...
	for _, path := range []string{
		"/api/admin/v1/permission/authplatform?page=1",
		"/api/admin/v1/permission/authplatform?page=1&page=2&pageSize=20",
		"/api/admin/v1/permission/authplatform?page=1&pageSize=20&unknown=1",
		"/api/admin/v1/permission/authplatform?page=0&pageSize=20",
		"/api/admin/v1/permission/authplatform?page=1&pageSize=101",
	} {
		recorder = httptest.NewRecorder()
...
func TestManagementJSONContractsRejectMissingAndUnknownFields(t *testing.T) {
	...
	recorder := performJSON(router, http.MethodPost, "/api/admin/v1/permission/authplatform", validCreate)
	...
	for _, body := range []string{
		...
		recorder = performJSON(router, http.MethodPost, "/api/admin/v1/permission/authplatform", body)
		if recorder.Code != http.StatusBadRequest {
	...
	recorder = performJSON(router, http.MethodPut, "/api/admin/v1/permission/authplatform/2", validUpdate)
	...
	for _, extra := range []string{`"code":"changed"`, `"isBuiltin":1`, `"policyVersion":2`} {
		...
		recorder = performJSON(router, http.MethodPut, "/api/admin/v1/permission/authplatform/2", body)
		if recorder.Code != http.StatusBadRequest {
			t.Fatalf("update with %s status=%d body=%s", extra, recorder.Code, recorder.Body)
		}
	}

	recorder = performJSON(router, http.MethodPatch, "/api/admin/v1/permission/authplatform/2/status", `{"isEnabled":0}`)
	...
	recorder = performJSON(router, http.MethodPatch, "/api/admin/v1/permission/authplatform/2/status", `{"isEnabled":0,"name":"bad"}`)
	if recorder.Code != http.StatusBadRequest {
...
func TestManagementDeleteRequiresEmptyBodyAndDeploymentIsSafe(t *testing.T) {
	service, router := managementRouter(t)
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, httptest.NewRequest(http.MethodDelete, "/api/admin/v1/permission/authplatform/2", nil))
	...
	recorder = performJSON(router, http.MethodDelete, "/api/admin/v1/permission/authplatform/2", `{}`)
	...
	recorder = httptest.NewRecorder()
	router.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/api/admin/v1/permission/authplatform/deployment", nil))
