...
func TestPlatformTableName(t *testing.T) {
	if got := (authplatform.Platform{}).TableName(); got != "permission_auth_platform" {
...
func TestAuthenticationPlatformSchemaAndBuiltinAdmin(t *testing.T) {
	...
	if err := connection.GORM.WithContext(ctx).Raw(
		`SELECT to_regclass(current_schema() || '.sys_permission_auth_platform') IS NOT NULL`,
	).Scan(&legacyTableExists).Error; err != nil {
		t.Fatal(err)
	}
	if legacyTableExists {
		t.Fatal("legacy relation sys_permission_auth_platform still exists")
	}
	if err := authplatform.EnsureSchema(ctx, connection.GORM); err != nil {
	...
	for column, want := range expectedColumns {
		...
		if err := connection.GORM.WithContext(ctx).Raw(`
			...
			WHERE table_schema = current_schema() AND table_name = 'permission_auth_platform' AND column_name = ?`, column).Scan(&got).Error; err != nil {
			t.Fatal(err)
		}
	...
	constraints := map[string][]string{
		"ck_permission_auth_platform_code":                      {"CHECK", "code"},
		"ck_permission_auth_platform_policy_version":            {"CHECK", "policy_version", ">= 1"},
		"ck_permission_auth_platform_access_ttl_seconds":        {"CHECK", "access_ttl_seconds", "2592000"},
		"ck_permission_auth_platform_refresh_ttl_seconds":       {"CHECK", "refresh_ttl_seconds", "31536000"},
		"ck_permission_auth_platform_session_cache_ttl_seconds": {"CHECK", "session_cache_ttl_seconds", "86400"},
		"ck_permission_auth_platform_access_cache_ttl_seconds":  {"CHECK", "access_cache_ttl_seconds", "86400"},
		"ck_permission_auth_platform_bind_device":               {"CHECK", "bind_device"},
		"ck_permission_auth_platform_bind_ip":                   {"CHECK", "bind_ip"},
		"ck_permission_auth_platform_max_sessions":              {"CHECK", "max_sessions", "100"},
		"ck_permission_auth_platform_allow_register":            {"CHECK", "allow_register"},
		"ck_permission_auth_platform_is_enabled":                {"CHECK", "is_enabled"},
		"ck_permission_auth_platform_is_builtin":                {"CHECK", "is_builtin"},
	}
	...
	if err := connection.GORM.WithContext(ctx).Raw(`
		...
		WHERE schemaname = current_schema() AND indexname = 'ux_permission_auth_platform_code_active'`).Scan(&indexDefinition).Error; err != nil {
		t.Fatal(err)
	}
