...
func TestRoleSchema(t *testing.T) {
	...
	if got := (role.Role{}).TableName(); got != "permission_role" {
		t.Fatalf("Role.TableName() = %q", got)
	}
	if got := (role.UserRole{}).TableName(); got != "permission_user_role" {
		t.Fatalf("UserRole.TableName() = %q", got)
	}

	for name, fragments := range map[string][]string{
		"ux_permission_role_code_active":    {"CREATE UNIQUE INDEX", "(code)", "deleted_at IS NULL"},
		"ux_permission_role_name_active":    {"CREATE UNIQUE INDEX", "(name)", "deleted_at IS NULL"},
		"ux_permission_role_default_active": {"CREATE UNIQUE INDEX", "(is_default)", "is_default = 1", "deleted_at IS NULL"},
		"ux_permission_user_role_active":    {"CREATE UNIQUE INDEX", "(user_id, role_id)", "deleted_at IS NULL"},
	} {
	...
	for name, fragments := range map[string][]string{
		"fk_permission_user_role_user":  {"FOREIGN KEY (user_id)", "user_account(id)", "ON DELETE RESTRICT"},
		"fk_permission_user_role_role":  {"FOREIGN KEY (role_id)", "permission_role(id)", "ON DELETE RESTRICT"},
		"ck_permission_role_is_default": {"CHECK", "is_default", "0", "1"},
		"ck_permission_role_is_enabled": {"CHECK", "is_enabled", "0", "1"},
	} {
...
func openRoleSchema(t *testing.T) (*gorm.DB, context.Context) {
	...
	if err := database.AutoMigrate(ctx, db, &account.User{}, &role.Role{}, &role.UserRole{}); err != nil {
