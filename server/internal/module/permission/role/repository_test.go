...
func TestFindDefaultRequiresExactlyOneEnabledRole(t *testing.T) {
	...
	t.Run("rejects multiple enabled defaults", func(t *testing.T) {
		...
		if err := tx.WithContext(ctx).Exec("DROP INDEX ux_permission_role_default_active").Error; err != nil {
			t.Fatal(err)
		}
...
func TestRepositoryListUsesStableFiltersAndExactCounts(t *testing.T) {
	...
	componentPath := "access/menus/" + unique
	path := "/" + componentPath
...
func openRoleDatabase(t *testing.T) (*gorm.DB, context.Context) {
	...
	if err := database.AutoMigrate(ctx, db, &account.User{}, &role.Role{}, &role.UserRole{}, &menu.Menu{}, &menu.RoleMenu{}, &auth.Session{}, &permission.Version{}); err != nil {
...
func createRoleAccessUser(t *testing.T, tx *gorm.DB, ctx context.Context, roleID int64, enabled yesno.Value, deleted bool) account.User {
	...
	if err := tx.WithContext(ctx).Create(&permission.Version{UserID: created.ID, Version: 1}).Error; err != nil {
		t.Fatal(err)
	}
