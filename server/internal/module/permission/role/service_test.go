...
func TestServiceUpdateStatusProtectsRolesAndPreservesRelations(t *testing.T) {
	...
	if err := tx.WithContext(ctx).Create(&permission.Version{UserID: createdUser.ID, Version: 1}).Error; err != nil {
		t.Fatal(err)
	}
...
func TestServiceUpdateStatusRollbackRestoresAccessStateAndVersion(t *testing.T) {
	...
	boundUser := createRoleAccessUser(t, tx, ctx, roleID, yesno.Yes, false)
	if err := tx.WithContext(ctx).Exec(`ALTER TABLE permission_role ADD CONSTRAINT ck_test_role_status_rollback CHECK (is_enabled = 1)`).Error; err != nil {
		t.Fatal(err)
	}
	if err := service.UpdateStatus(ctx, roleID, yesno.No); roleErrorCode(err) != apperror.CodeDependencyUnavailable {
		t.Fatalf("UpdateStatus() error = %v", err)
...
func TestConcurrentRoleMutationRechecksChangedCandidate(t *testing.T) {
	...
	waitForRoleAccessState(t, accessStates, boundUser.ID, permissionstate.StateInvalidating)
...
func TestServiceUpdateStatusPublishFailureLeavesCommittedVersionUnreachable(t *testing.T) {
	...
	if err := db.WithContext(ctx).Exec(`
		...
		BEFORE UPDATE OF is_enabled ON permission_role
	...
	go func() { done <- service.UpdateStatus(ctx, roleID, yesno.No) }()
	waitForRoleAccessState(t, accessStates, boundUser.ID, permissionstate.StateInvalidating)
	if err := redisClient.Delete(ctx, permissionstate.StateKey(boundUser.ID)); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-done:
...
func TestServiceDeleteRollsBackWhenRoleWriteFails(t *testing.T) {
	...
	if err := tx.WithContext(ctx).Exec(`ALTER TABLE permission_role ADD CONSTRAINT ck_test_role_delete_rollback CHECK (deleted_at IS NULL)`).Error; err != nil {
		t.Fatal(err)
	}
...
func newRoleMutationTestService(t *testing.T, repository *role.Repository) (*role.Service, *permissionstate.Store, *projectredis.Client) {
	t.Helper()
	redisClient := openRoleTestRedis(t)
	accessStates := permissionstate.NewStore(redisClient)
	return role.NewService(repository, permissionstate.NewInvalidator(accessStates)), accessStates, redisClient
}
...
func assertRoleAccessState(t *testing.T, store *permissionstate.Store, userID, version int64) {
	t.Helper()
...
func waitForRoleAccessState(t *testing.T, store *permissionstate.Store, userID int64, wanted string) {
	t.Helper()
