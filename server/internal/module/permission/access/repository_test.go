...
func TestFindSourceWithVersionLoadsOneAuthoritativeRBACFact(t *testing.T) {
	...
	if len(source.Menus) != 3 || source.Menus[0].MenuType != permission.MenuDirectory {
...
func TestFindSourceWithVersionRejectsUnavailableUserOrMissingVersion(t *testing.T) {
	for _, mutate := range []func(*accessRepositoryFixture) error{
		...
		func(fixture *accessRepositoryFixture) error {
			return fixture.db.WithContext(fixture.ctx).Delete(&permission.Version{}, fixture.user.ID).Error
		},
	} {
		fixture := openAccessRepositoryFixture(t)
...
func openAccessRepositoryFixture(t *testing.T) *accessRepositoryFixture {
	...
	if err := database.AutoMigrate(ctx, connection.GORM,
		&account.User{}, &role.Role{}, &role.UserRole{}, &menu.Menu{}, &menu.RoleMenu{}, &permission.Version{}); err != nil {
		t.Fatal(err)
	}
	...
	if err := connection.GORM.WithContext(ctx).Create(&permission.Version{UserID: createdUser.ID, Version: 3, CreatedAt: now, UpdatedAt: now}).Error; err != nil {
		t.Fatal(err)
	}
	...
	return &accessRepositoryFixture{
		db: connection.GORM, ctx: ctx, repository: permission.NewRepository(connection.GORM), user: createdUser,
