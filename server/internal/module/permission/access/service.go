...
const CodePermissionSnapshotInvalid = 14000
...
func (s *Service) loadSnapshot(ctx context.Context, identity auth.Identity) (Snapshot, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if current.State == permissionstate.StateInvalidating {
			return Snapshot{}, accessUpdating(permissionstate.ErrUpdating)
		}
...
func accessSnapshotInvalid(cause error) *apperror.Error {
	return &apperror.Error{
		HTTPStatus: http.StatusInternalServerError, Code: CodePermissionSnapshotInvalid,
		MessageKey: i18n.KeyPermissionSnapshotInvalid, Cause: cause,
	}
}
