...
func TestAccessHandlerReturnsClosedSnapshot(t *testing.T) {
	...
	service := &currentAccessService{snapshot: permission.Snapshot{
...
func TestAccessHandlerRejectsMissingIdentity(t *testing.T) {
	...
	router := gin.New()
	router.GET("/access", permission.NewHandler(service).Current)
...
func (s *currentAccessService) Current(ctx context.Context, identity auth.Identity) (permission.Snapshot, error) {
	s.calls++
	s.ctx = ctx
	s.identity = identity
...
func serveAccessRoute(t *testing.T, service *currentAccessService, authorization string) *httptest.ResponseRecorder {
	...
	gin.SetMode(gin.TestMode)
	router := gin.New()
	permission.RegisterRoutes(router.Group("/api/v1", authclient.Require()), permission.NewHandler(service), auth.Authenticate(accessAuthService{}))
