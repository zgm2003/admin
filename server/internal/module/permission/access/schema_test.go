...
func TestAccessVersionTableName(t *testing.T) {
	if got := (permission.Version{}).TableName(); got != "permission_access_version" {
...
func TestAccessVersionSchemaBackfillsUsers(t *testing.T) {
	...
	if err := database.AutoMigrate(ctx, connection.GORM, &permission.Version{}); err != nil {
		t.Fatal(err)
	}
	if err := permission.EnsureSchema(ctx, connection.GORM); err != nil {
		t.Fatal(err)
	}
	...
	for column, wantType := range map[string]string{
		...
		if err := connection.GORM.WithContext(ctx).Raw(`
			...
			WHERE table_schema = current_schema() AND table_name = 'permission_access_version' AND column_name = ?`, column).Scan(&got).Error; err != nil {
			t.Fatal(err)
		}
	...
	if err := connection.GORM.WithContext(ctx).Raw(`
		...
		WHERE table_schema = current_schema() AND table_name = 'permission_access_version' AND column_name = 'deleted_at'`).Scan(&deletedAtCount).Error; err != nil {
		t.Fatal(err)
	}
	if deletedAtCount != 0 {
		t.Fatal("permission_access_version must not have deleted_at")
	}
	for name, fragments := range map[string][]string{
		"ck_permission_access_version_version": {"CHECK", "version", ">= 1"},
		"fk_permission_access_version_user":    {"FOREIGN KEY", "user_id", "ON DELETE RESTRICT"},
	} {
