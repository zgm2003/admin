...
func TestRequirePermissionRejectsMissingIdentityWithoutCallingService(t *testing.T) {
	...
	recorder, called := servePermissionRequest(t, service, false, "user:account:create")
...
func TestRequirePermissionAllowsTheNextHandler(t *testing.T) {
	...
	recorder, called := servePermissionRequest(t, service, true, "user:account:create")
	if recorder.Code != http.StatusNoContent || !called {
		t.Fatalf("status=%d next=%v body=%s", recorder.Code, called, recorder.Body)
	}
	if service.identity.UserID != 41 || service.identity.PlatformID != 17 || service.identity.Platform != "admin" || service.code != "user:account:create" || service.ctx == nil {
...
func TestRequirePermissionReturnsTranslatedForbidden(t *testing.T) {
	...
	recorder, called := servePermissionRequest(t, service, true, "user:account:delete")
