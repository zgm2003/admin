...
func (r *Repository) Increment(ctx context.Context, userIDs []int64, now time.Time) (map[int64]int64, error) {
	...
	if err := r.db.WithContext(ctx).Raw("UPDATE permission_access_version SET version=version+1, updated_at=? WHERE user_id IN ? RETURNING user_id, version", now.UTC(), dedup).Scan(&rows).Error; err != nil {
