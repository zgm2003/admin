...
func PrepareSchema(ctx context.Context, db *gorm.DB) error {
	...
	var exists bool
	if err := db.Raw(`SELECT to_regclass(current_schema() || '.permission_menu') IS NOT NULL`).Scan(&exists).Error; err != nil {
...
func PreparePlatformSchema(ctx context.Context, db *gorm.DB) error {
	...
	if err := db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Exec(`ALTER TABLE permission_menu ADD COLUMN IF NOT EXISTS platform_id BIGINT`).Error; err != nil {
			return fmt.Errorf("add menu platform column: %w", err)
		}
		if err := tx.Exec(`UPDATE permission_menu SET platform_id = ? WHERE platform_id IS NULL`, adminIDs[0]).Error; err != nil {
			return fmt.Errorf("backfill menu platform column: %w", err)
		}
		if err := tx.Exec(`ALTER TABLE permission_menu ALTER COLUMN platform_id SET NOT NULL`).Error; err != nil {
...
func prepareLegacyMenuCatalog(db *gorm.DB) error {
	...
	if err := db.Raw(`SELECT EXISTS (SELECT 1 FROM permission_menu WHERE code = 'access')`).Scan(&accessExists).Error; err != nil {
	...
	for _, targetCode := range append([]string{"account", "access"}, currentPermissionCodes()...) {
		...
		var occupied bool
		if err := db.Raw(`SELECT EXISTS (SELECT 1 FROM permission_menu WHERE code = ?)`, targetCode).Scan(&occupied).Error; err != nil {
	...
	if err := db.Exec(`
		UPDATE permission_role_menu AS role_menu
	...
	for _, row := range rows {
		...
		if err := db.Exec(`UPDATE permission_menu SET code = ? WHERE id = ?`, temporaryCode, row.ID).Error; err != nil {
	...
	for _, code := range []string{"account", "system"} {
		var id int64
		if err := db.Raw(`SELECT id FROM permission_menu WHERE code = ?`, code).Scan(&id).Error; err != nil || id < 1 {
	...
	if err := db.Exec(`ALTER TABLE permission_menu DROP COLUMN IF EXISTS view_key`).Error; err != nil {
...
func rekeyMigratedOperationLogMenu(db *gorm.DB) error {
	...
	if err := db.Raw(`SELECT count(*) FROM permission_menu WHERE code = 'audit:operation-log:list'`).Scan(&legacyCount).Error; err != nil {
...
func legacyMenuViewExpression(db *gorm.DB) (string, error) {
	...
	if err := db.Raw(`
		SELECT EXISTS (
			...
			WHERE table_schema = current_schema() AND table_name = 'permission_menu' AND column_name = 'view_key'
	...
	if err := db.Raw(`
		SELECT EXISTS (
			SELECT 1 FROM information_schema.columns
			WHERE table_schema = current_schema() AND table_name = 'permission_menu' AND column_name = 'component_path'
