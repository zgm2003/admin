...
func TestPrepareSchemaRekeysLegacyMenuCatalogInPlace(t *testing.T) {
	...
	if access.I18nKey == nil || *access.I18nKey != "navigation.permission" ||
		access.Icon == nil || *access.Icon != "lucide:shield-check" {
		t.Fatalf("access root render fields = %+v", access)
	}
	assertRekeyedPage(t, byCode["user:account:view"], account.ID, "/user/account", "user/account", "navigation.userAccount", "lucide:user-round-cog", 10)
	...
	customName := "自定义权限中心"
	if err := db.Exec(`UPDATE permission_menu SET name = ?, sort_order = 260 WHERE code = 'access'`, customName).Error; err != nil {
		t.Fatal(err)
	}
	if err := menu.PrepareSchema(ctx, db); err != nil {
	...
	if err := db.Raw(`SELECT name, sort_order FROM permission_menu WHERE code = 'access'`).Scan(&repeated).Error; err != nil {
		t.Fatal(err)
	}
...
func TestPrepareSchemaRejectsUnknownLegacyIconAndRollsBack(t *testing.T) {
	...
	if err := db.Exec(`UPDATE permission_menu SET icon = 'UnknownIcon' WHERE code = 'system:session:list'`).Error; err != nil {
		t.Fatal(err)
	}
	...
	var code string
	if err := db.Raw(`SELECT code FROM permission_menu WHERE id = 1`).Scan(&code).Error; err != nil {
		t.Fatal(err)
	}
	if code != "system" {
...
func TestPrepareSchemaRepairsCorruptedAccessRootI18nKey(t *testing.T) {
	...
	if stored.I18nKey == nil || *stored.I18nKey != "navigation.permission" {
...
func readRekeyedMenus(t *testing.T, db *gorm.DB) []rekeyedMenuRow {
	...
	if err := db.Raw(`
		...
		FROM permission_menu ORDER BY id`).Scan(&rows).Error; err != nil {
		t.Fatal(err)
	}
	return rows
}
...
func assertRoleMenuDeletion(t *testing.T, db *gorm.DB, id int64, want *time.Time) {
	...
	if err := db.Raw(`SELECT deleted_at FROM permission_role_menu WHERE id = ?`, id).Scan(&got).Error; err != nil {
		t.Fatal(err)
	}
...
func assertAccessVersions(t *testing.T, db *gorm.DB, want map[int64]int64) {
	...
	if err := db.Raw(`SELECT user_id, version FROM permission_access_version ORDER BY user_id`).Scan(&rows).Error; err != nil {
		t.Fatal(err)
	}
...
func openDomainRekeySchema(t *testing.T, suffix string) (*gorm.DB, context.Context, time.Time, time.Time) {
	...
	for _, statement := range []string{
		...
		`CREATE UNIQUE INDEX ux_permission_menu_code_active ON permission_menu (code) WHERE deleted_at IS NULL`,
	...
	for _, statement := range []struct {
		...
		if err := tx.Exec(`INSERT INTO permission_role_menu (id, role_id, menu_id, deleted_at) VALUES (?, 9001, ?, ?)`, statement.id, statement.menuID, statement.deletedAt).Error; err != nil {
			t.Fatal(err)
		}
	}
	if err := tx.Exec(`INSERT INTO permission_access_version (user_id, version) VALUES (7001, 4), (7002, 7)`).Error; err != nil {
		t.Fatal(err)
	}
	if err := tx.Exec(`SELECT setval(pg_get_serial_sequence('permission_menu', 'id'), 25, true)`).Error; err != nil {
		t.Fatal(err)
	}
