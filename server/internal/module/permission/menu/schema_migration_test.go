...
func TestMenuEnsureSchemaPreservesActiveCatalogAndGrants(t *testing.T) {
	...
	if err := db.Exec(`ALTER TABLE permission_menu ADD COLUMN view_key VARCHAR(128) NULL`).Error; err != nil {
...
func TestPreparePlatformSchemaBackfillsHistoryAndPreservesGrants(t *testing.T) {
	...
	for _, statement := range []string{
		`DROP INDEX IF EXISTS ux_permission_menu_code_active`,
		`DROP INDEX IF EXISTS ux_permission_menu_page_path_active`,
		`DROP INDEX IF EXISTS ix_permission_menu_parent_active`,
		`ALTER TABLE permission_menu DROP CONSTRAINT IF EXISTS fk_permission_menu_parent_platform`,
		`ALTER TABLE permission_menu DROP CONSTRAINT IF EXISTS uq_permission_menu_id_platform`,
		`ALTER TABLE permission_menu DROP CONSTRAINT IF EXISTS fk_permission_menu_platform`,
		`ALTER TABLE permission_menu DROP COLUMN platform_id`,
	} {
	...
	if err := db.Unscoped().Table("permission_menu").Select("id, platform_id").Where("id IN ?", []int64{activeID, deletedID}).Order("id").Scan(&rows).Error; err != nil {
		t.Fatal(err)
	}
...
func TestMenuSchemaRejectsCrossPlatformParent(t *testing.T) {
	...
	if !errors.As(err, &postgresError) || postgresError.ConstraintName != "fk_permission_menu_parent_platform" {
