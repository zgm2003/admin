...
func TestMenuSchema(t *testing.T) {
	...
	if got := (menu.Menu{}).TableName(); got != "permission_menu" {
		t.Fatalf("Menu.TableName() = %q", got)
	}
	if got := (menu.RoleMenu{}).TableName(); got != "permission_role_menu" {
	...
	assertColumnMissing(t, connection, ctx, "permission_menu", "view_key")
	...
	constraints := map[string][]string{
		...
		"fk_permission_role_menu_menu":       {"FOREIGN KEY", "ON DELETE RESTRICT"},
	}
	...
	assertConstraintMissing(t, connection, ctx, "fk_permission_menu_parent")

	indexes := map[string][]string{
		"ux_permission_menu_code_active":      {"CREATE UNIQUE INDEX", "(platform_id, code)", "WHERE (deleted_at IS NULL)"},
		"ux_permission_menu_page_path_active": {"CREATE UNIQUE INDEX", "(platform_id, path)", "menu_type", "page", "deleted_at IS NULL"},
		"ix_permission_menu_parent_active":    {"CREATE INDEX", "(platform_id, parent_id, sort_order, id)", "WHERE (deleted_at IS NULL)"},
		"ux_permission_role_menu_active":      {"CREATE UNIQUE INDEX", "(role_id, menu_id)", "WHERE (deleted_at IS NULL)"},
	}
