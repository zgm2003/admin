...
func TestCOSObjectStorageMigration(t *testing.T) {
	...
	for _, name := range []string{"fk_storage_upload_rule_platform", "fk_storage_upload_rule_cos_config", "ck_storage_cos_config_is_enabled", "ck_storage_upload_rule_is_enabled", "ck_storage_upload_rule_max_file_size", "ck_storage_upload_rule_max_file_count", "ck_storage_upload_rule_access_mode"} {
		assertNamedConstraint(t, db, ctx, name)
	}
	for _, name := range []string{"ux_storage_cos_config_name_active", "ix_storage_cos_config_enabled_created_at", "ux_storage_upload_rule_platform_code_active", "ux_storage_upload_rule_platform_enabled", "ix_storage_upload_rule_config_enabled_created_at"} {
	...
	if err := db.WithContext(ctx).Exec(`INSERT INTO storage_upload_rule (platform_id, code, name, cos_config_id, path_prefix, max_file_size_bytes, max_file_count, allowed_extensions, allowed_mime_types, access_mode) VALUES (?, 'avatar', 'Avatar', ?, 'avatars', 100, 1, ARRAY['png']::text[], ARRAY['image/png']::text[], 'private')`, admin.ID, configID).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.WithContext(ctx).Exec(`INSERT INTO storage_upload_rule (platform_id, code, name, cos_config_id, path_prefix, max_file_size_bytes, max_file_count, allowed_extensions, allowed_mime_types, access_mode, is_enabled) VALUES (?, 'avatar-disabled', 'Disabled', ?, 'avatars', 100, 1, ARRAY['png']::text[], ARRAY['image/png']::text[], 'private', 0)`, admin.ID, configID).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.WithContext(ctx).Exec(script).Error; err != nil {
		t.Fatal(err)
...
func TestCOSObjectStorageMigrationEnforcesEnabledRuleUniqueness(t *testing.T) {
	db, ctx := openCOSMigrationSchema(t)
	admin := ensureCOSAdmin(t, db, ctx)
	...
	insert := `INSERT INTO storage_upload_rule (platform_id, code, name, cos_config_id, path_prefix, max_file_size_bytes, max_file_count, allowed_extensions, allowed_mime_types, access_mode) VALUES (?, ?, ?, ?, 'avatars', 100, 1, ARRAY['png']::text[], ARRAY['image/png']::text[], ?)`
	...
	if err := db.WithContext(ctx).Exec(insert, admin.ID, "two", "Two", configID, "private").Error; err == nil {
		t.Fatal("second enabled rule was accepted")
	}
	disabledInsert := `INSERT INTO storage_upload_rule (platform_id, code, name, cos_config_id, path_prefix, max_file_size_bytes, max_file_count, allowed_extensions, allowed_mime_types, access_mode, is_enabled) VALUES (?, ?, ?, ?, 'avatars', 100, 1, ARRAY['png']::text[], ARRAY['image/png']::text[], ?, 0)`
