...
`docs/local-work/**` 是不提交的本地临时 Spec/Plan 区。除 `README.md` 外均由 Git 忽略；Agent 默认不得扫描、
读取或引用其中内容，只有维护者明确指定文件或明确要求继续当前本地计划时才可读取。完成后由维护者删除。
...
- 缓存、限流、队列和失效协议必须明确跨实例一致性、故障行为和回源上限；
- 跨模块 spec/plan 必须写明容量假设、热点查询预算、缓存更新策略和并发验证方式。
