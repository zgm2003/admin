...
INSERT INTO permission_menu(platform_id,parent_id,menu_type,name,code,i18n_key,path,component_path,sort_order,is_enabled,is_hidden)
SELECT p.id,d.id,'page','邮件服务','message:mail:view','navigation.mail','/message/mail','message/mail',10,1,0 FROM auth_platform p JOIN permission_menu d ON d.platform_id=p.id AND d.code='message' AND d.menu_type='directory' AND d.deleted_at IS NULL WHERE p.code='admin' AND p.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM permission_menu m WHERE m.platform_id=p.id AND m.code='message:mail:view' AND m.deleted_at IS NULL);
WITH a(code,name) AS (VALUES ('message:mail:list','读取邮件服务'),('message:mail:detail','读取邮件详情'),('message:mail:config:update','修改邮件配置'),('message:mail:config:delete','删除邮件配置'),('message:mail:test','发送测试邮件'),('message:mail:template:update','修改邮件模板'),('message:mail:template:status','启停邮件模板'),('message:mail:log:delete','删除邮件日志'),('message:mail:rule:create','新增收件人规则'),('message:mail:rule:update','修改收件人规则'),('message:mail:rule:status','启停收件人规则'),('message:mail:rule:delete','删除收件人规则'))
INSERT INTO permission_menu(platform_id,parent_id,menu_type,name,code,sort_order,is_enabled,is_hidden)
SELECT p.id,m.id,'action',a.name,a.code,10,1,1 FROM auth_platform p JOIN permission_menu m ON m.platform_id=p.id AND m.code='message:mail:view' AND m.deleted_at IS NULL CROSS JOIN a WHERE p.code='admin' AND p.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM permission_menu x WHERE x.platform_id=p.id AND x.code=a.code AND x.deleted_at IS NULL);
COMMIT;
