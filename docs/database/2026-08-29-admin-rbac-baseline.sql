...
BEGIN
  ...
  IF old_canvas_count = 1 THEN
    UPDATE permission_menu m SET code = 'canvas:test:list', updated_at = CURRENT_TIMESTAMP
  ...
  IF EXISTS (SELECT 1 FROM permission_menu WHERE platform_id = admin_platform_id AND code IN ('account:profile:update', 'account:password:update') AND deleted_at IS NULL GROUP BY code HAVING count(*) <> 1) THEN
  ...
  IF (SELECT count(*) FROM permission_menu WHERE platform_id = admin_platform_id AND code = 'account:user:loginlog:list' AND deleted_at IS NULL) > 1 THEN
  ...
  WHERE platform_id = admin_platform_id AND code = 'permission:menu:list' AND deleted_at IS NULL;
