...
SET code       = regexp_replace(code, ':list$', ':view'),
...
WHERE code = 'admin'
  ...
  AND is_enabled = 1;
IF
admin_platform_id IS NULL OR (SELECT count(*) FROM auth_platform WHERE code = 'admin' AND deleted_at IS NULL AND is_enabled = 1) <> 1 THEN
...
WHERE platform_id = admin_platform_id
  ...
  AND deleted_at IS NULL;
IF
account_id IS NULL OR (SELECT count(*) FROM permission_menu WHERE platform_id = admin_platform_id AND code = 'account' AND deleted_at IS NULL) <> 1 THEN
    RAISE EXCEPTION 'Admin account directory is missing or duplicated';
END IF;

SELECT count(*)
INTO old_canvas_count
FROM permission_menu m
         JOIN auth_platform p ON p.id = m.platform_id
WHERE p.code = 'canvas'
  AND p.deleted_at IS NULL
  AND m.code = 'canvas:test'
  AND m.deleted_at IS NULL;
SELECT count(*)
INTO new_canvas_count
FROM permission_menu m
         JOIN auth_platform p ON p.id = m.platform_id
WHERE p.code = 'canvas'
  AND p.deleted_at IS NULL
  AND m.code = 'canvas:test:view'
  AND m.deleted_at IS NULL;
IF
old_canvas_count > 1 OR new_canvas_count > 1 OR (old_canvas_count > 0 AND new_canvas_count > 0) THEN
    RAISE EXCEPTION 'Canvas page code conflict';
END IF;
  IF
old_canvas_count = 1 THEN
UPDATE permission_menu m
SET code       = 'canvas:test:view',
    updated_at = CURRENT_TIMESTAMP FROM auth_platform p
...
new_canvas_count = 1 THEN
    ...
    ) THEN RAISE EXCEPTION 'Canvas target page shape mismatch';
END IF;
END IF;
...
FROM permission_menu
WHERE platform_id = admin_platform_id
  AND code = 'account:profile:view'
  AND deleted_at IS NULL;
IF
profile_id IS NOT NULL THEN
...
END IF;
  ...
  IF
EXISTS (SELECT 1 FROM permission_menu WHERE platform_id = admin_platform_id AND code IN ('account:profile:update', 'account:password:update', 'account:profile:detail') AND deleted_at IS NULL GROUP BY code HAVING count(*) <> 1) THEN
    RAISE EXCEPTION 'profile action code conflict';
END IF;
  IF
NOT EXISTS (SELECT 1 FROM permission_menu WHERE parent_id = profile_id AND code = 'account:profile:update' AND deleted_at IS NULL) THEN
    INSERT INTO permission_menu (platform_id, parent_id, menu_type, name, code, i18n_key, path, component_path, icon, sort_order, is_enabled, is_hidden, created_at, updated_at)
...
END IF;
  IF
NOT EXISTS (SELECT 1 FROM permission_menu WHERE parent_id = profile_id AND code = 'account:password:update' AND deleted_at IS NULL) THEN
    INSERT INTO permission_menu (platform_id, parent_id, menu_type, name, code, i18n_key, path, component_path, icon, sort_order, is_enabled, is_hidden, created_at, updated_at)
...
FROM permission_menu
WHERE platform_id = admin_platform_id
  AND code = 'account:user:loginlog:view'
  AND deleted_at IS NULL;
IF
(
SELECT count(*)
...
FROM permission_menu
WHERE platform_id = admin_platform_id
  AND code = 'permission:menu:view'
  AND deleted_at IS NULL;
IF
menu_page_id IS NULL THEN
    RAISE EXCEPTION 'menu management page is missing';
END IF;

FOR read_definition IN
SELECT *
FROM (VALUES ('permission:menu:view', 'permission:menu:list', '读取菜单列表', 5),
             ...
             ('account:profile:view', 'account:profile:detail', '读取个人资料',
              5)) AS definitions(parent_code, action_code, action_name, sort_order) LOOP
...
FROM permission_menu
WHERE platform_id = admin_platform_id
  AND code = read_definition.parent_code
  AND menu_type = 'page'
  AND deleted_at IS NULL;
IF
read_page_id IS NULL THEN
      CONTINUE;
END IF;
    IF
NOT EXISTS (SELECT 1 FROM permission_menu WHERE parent_id = read_page_id AND code = read_definition.action_code AND deleted_at IS NULL) THEN
...
WHERE (granted.id = read_page_id OR granted.parent_id = read_page_id)
  AND NOT EXISTS (SELECT 1
                  FROM permission_role_menu existing
                  WHERE existing.role_id = grants.role_id
                    AND existing.menu_id = read_action_id
...
FROM permission_menu
WHERE platform_id = admin_platform_id
  AND code = 'permission:access-cache:rebuild'
  AND deleted_at IS NULL) > 1 THEN
    RAISE EXCEPTION 'access cache rebuild action code conflict';
END IF;
  IF
NOT EXISTS (SELECT 1 FROM permission_menu WHERE platform_id = admin_platform_id AND parent_id = menu_page_id AND code = 'permission:access-cache:rebuild' AND deleted_at IS NULL) THEN
    INSERT INTO permission_menu (platform_id, parent_id, menu_type, name, code, i18n_key, path, component_path, icon, sort_order, is_enabled, is_hidden, created_at, updated_at)
...
GET DIAGNOSTICS changed_rows = ROW_COUNT;
changed
:= changed OR changed_rows > 0;

  IF
changed THEN
UPDATE permission_access_version v
SET version    = v.version + 1,
    updated_at = CURRENT_TIMESTAMP FROM user_account u
