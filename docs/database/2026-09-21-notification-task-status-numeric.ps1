<#
.SYNOPSIS
    Converts notification task status strings to numeric enum values.
#>
[CmdletBinding()]
...
$sqlPath = Join-Path $PSScriptRoot '2026-09-21-notification-task-status-numeric.sql'
...
$before = Query $dsn "SELECT data_type || ':' || COALESCE(column_default,'<none>') FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='message_notification_task' AND column_name='status';"
if ($before -notmatch '^character varying:' -and $before -notmatch '^text:' -and $before -notmatch '^smallint:') {
    throw "拒绝执行：notification task status 类型不受支持：$before"
}

$backupDir = Join-Path $env:LOCALAPPDATA ('Admin\backups\notification-task-status-numeric-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
$facts = Query $dsn "SELECT (SELECT data_type FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='message_notification_task' AND column_name='status') || ':' || (SELECT column_default FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='message_notification_task' AND column_name='status') || ':' || (SELECT count(*) FROM message_notification_task WHERE status NOT BETWEEN 1 AND 7) || ':' || (SELECT count(*) FROM pg_constraint WHERE conrelid='message_notification_task'::regclass AND conname IN ('ck_message_notification_task_status','ck_message_notification_task_state'));"
if ($facts -ne 'smallint:1:0:2') { throw "验证失败：$facts" }
...
Write-Host "完成：notificationTask 状态数值化、备份、只读验证和重复执行通过。事实：$facts"
