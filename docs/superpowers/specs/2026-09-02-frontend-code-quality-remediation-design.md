...
`request.ts` 是接口错误通知的唯一所有者：对业务 `code != 0`、HTTP、网络和协议错误统一调用
Element Plus `ElNotification` 提示一次，同时继续抛出归一化错误。HTTP 401/403 不弹通用通知，401
由认证刷新/最终跳转处理，403 由权限页面或业务流程处理。认证失效后的清理和跳转可以通过明确
注入的认证回调协调。

页面错误展示遵守：

- 列表加载失败进入 AppTable 或页面 inline error 状态；
- 表单和 mutation 的接口失败由 request.ts 统一通知；页面只维护业务状态和成功反馈；
- 同一个接口失败只能由 request.ts 展示一次，不由页面重复弹出 notification；
- 成功通知、确认弹窗和取消语义继续由页面拥有；
- 页面可以为控制异步流程保留 catch，但不得在 catch 中重复提示同一接口错误。

修改请求层通知行为前必须盘点所有调用方，保证非 401/403 失败仍由 request.ts 可见且可测试；不得
删除请求层通知并要求每个页面自行补通知，也不得把“去重提示”实现成静默失败。
...
10. `utils/request.ts` 必须统一产生接口错误 `ElNotification`；仅 401/403 不产生通用通知。
...
4. 删除 `?? []` 等必填字段静默修复；
5. 保留并验证请求层统一 UI 通知，保证非 401/403 错误只提示一次且继续抛出。
