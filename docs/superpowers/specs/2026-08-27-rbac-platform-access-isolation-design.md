...
本切片为 Admin 后端建立多平台 RBAC 基线，并为已有 `canvas` 平台保留一个根级 `test` 页面和
一个页面下的 `test` 按钮权限，不创建目录，不写 Canvas 业务前端。平台和菜单的准备方式以
上方修订说明为准。
...
不执行 foundation、seed 或菜单插入。已有有效 Canvas 策略值不被启动流程重置；测试仍在独立
事务或测试 schema 内验证数据库行为。
