...
**Architecture:** 普通 HTTP 请求继续使用 router -> middleware -> handler -> service -> repository -> model -> PostgreSQL。操作日志在请求中间件完成原响应后采集并通过 Asynq 入队，Worker 使用 task handler -> service -> repository -> model -> PostgreSQL 追加写入；入队失败只记录结构化错误，不改变业务响应。会话管理复用 auth 的 Session、authstate 和 session snapshot 删除路径，撤销顺序固定为 PostgreSQL -> authstate invalidation -> Redis snapshot delete。
...
- [ ] Step 1: 在 schema_test.go 写真实 PostgreSQL 失败测试。AutoMigrate OperationLog 后查询 information_schema.columns 和 pg_indexes，断言字段齐全、request_id 唯一索引、created_at/action 查询索引存在；插入 is_success=2 必须被 CHECK 拒绝。
...
- [ ] Step 4: 实现 EnsureSchema，创建 is_success CHECK、request_id 唯一索引、created_at、user_created、action_created 三个真实查询索引；重复执行成功。API AutoMigrate 加入模型，并在现有 schema 初始化后调用 EnsureSchema。
...
- [ ] Step 4: 实现 JSON 摘要脱敏。递归处理 object/array，敏感字段不区分大小写替换固定掩码；超过固定最大字节数时只保留前缀并带 truncated 标记；解析失败不得保存原始 body。
...
- [ ] Step 1: 写失败测试 TestDecodePayloadRejectsUnknownOrMissingFields、TestTaskHandlerMarksMalformedPayloadSkipRetry、TestEnqueueUsesStableTaskIDAndBoundedRetry。覆盖未知字段、缺字段、错误类型、尾随 JSON、SkipRetry、任务类型 system:operation-log:v1、TaskID=request id、MaxRetry 和 Timeout。
- [ ] Step 2: 运行 cd server; go test ./internal/module/operationlog -run "TestDecodePayload|TestTaskHandler|TestEnqueue" -count=1，预期失败。
- [ ] Step 3: 定义闭合 TaskPayload：schemaVersion、requestId、userId、sessionId、platform、method、route、module、action、clientIp、userAgent、statusCode、isSuccess、latencyMs、requestData、responseData、createdAt。DisallowUnknownFields、单文档、必填和范围校验；非法 payload 包装 asynq.SkipRetry。
- [ ] Step 4: 合法任务调用 operationlog service 的 request_id 幂等插入；数据库错误返回让 Asynq 重试，request_id 冲突视为已处理。API 创建 QueueEnqueuer；Worker 创建 PostgreSQL repository/service 并调用 operationlog.Register(mux, service)，不使用内存队列。
...
- [ ] Step 4: Service 接收 actor user/session，拒绝当前 session；已撤销只计 skipped；数据库成功后复用 authstate invalidator 生成新 generation，再调用现有 session snapshot delete。Redis 删除失败返回 dependency unavailable，不回滚 PostgreSQL。
...
- [ ] Step 1: 写失败 wiring/integration 测试：新路由只注册一次且有权限 middleware；Worker 注册 system:operation-log:v1；真实 PostgreSQL、Redis、Asynq Worker 验证登记操作入库、撤销写入 revoked_at、删除 snapshot、旧 token 被拒绝。
...
- Asynq payload、SkipRetry、重试和 request_id 幂等：Task 3。
