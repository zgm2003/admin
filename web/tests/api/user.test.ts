...
describe('user API', () => {
  ...
  it('sends exact request shapes', async () => {
    ...
    requestMock.mockResolvedValueOnce({ id:7, username:'alice_new', updatedAt:'2026-08-20T00:00:00Z' }); await updateUser(7,{ username:'alice_new' }); expect(requestMock).toHaveBeenLastCalledWith({ method:'PUT', url:'/api/admin/v1/users/7', data:{ username:'alice_new' } })
    ...
    requestMock.mockResolvedValueOnce({ user:{id:7,username:'alice',email:'a@b.com',isEnabled:YesNo.Yes}, roles:[{id:2,code:'member',name:'Member',isEnabled:YesNo.Yes}], roleIds:[2] }); await getUserRoles(7); expect(requestMock).toHaveBeenLastCalledWith({ method:'GET', url:'/api/admin/v1/users/7/roles' })
