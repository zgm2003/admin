...
describe('authentication platform API', () => {
  ...
  it('uses exact mutation methods and allowlisted bodies', async () => {
    ...
    expect(requestMock).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ method: 'POST', url: '/api/admin/v1/permission/authplatform' }),
    )
    expect(requestMock.mock.calls[0]?.[0]).toMatchObject({
      data: expect.objectContaining({ code: 'portal' }),
    })
    expect(requestMock.mock.calls[1]?.[0]).toMatchObject({
      method: 'PUT',
      url: '/api/admin/v1/permission/authplatform/3',
    })
    expect(requestMock.mock.calls[1]?.[0]).not.toHaveProperty('data.code')
    expect(requestMock).toHaveBeenNthCalledWith(3, {
      method: 'PATCH',
      url: '/api/admin/v1/permission/authplatform/3/status',
