...
const action = (id: number, code: string, name: string): ManagedMenuNode => ({
  id,
  platformId: 1,
  platformCode: 'admin',
  platformName: 'Admin',
    ...
    children: [
      {
        ...
        updatedAt: '2026-08-19T02:00:00Z',
        children: [
          action(3, 'permission:menu:create', '新增菜单'),
          action(4, 'permission:menu:update', '修改菜单'),
        ],
      },
    ],
  },
...
describe('filterManagedMenuTree', () => {
  it('keeps matches, ancestors, and matched-node descendants without mutating input', () => {
    ...
    expect(result[0]?.children[0]?.children.map((node) => node.code)).toEqual([
  ...
  it('matches name and path case-insensitively, clones full tree for empty keyword, and returns no result', () => {
    ...
    expect(filterManagedMenuTree(tree(), '/PERMISSION/MENU')[0]?.children).toHaveLength(1)
