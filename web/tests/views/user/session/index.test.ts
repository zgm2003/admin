...
import SessionManagement from '@/views/user/session/index.vue'
...
describe('session management', () => {
  ...
  it('loads list and stats, applies filters, and changes pages', async () => {
    ...
    expect(wrapper.get('.session-page').classes()).toContain('management-page')
    ...
    wrapper.findComponent({ name: 'ElSelectV2' }).vm.$emit('update:modelValue', 'active')
  ...
  it('keeps the statistics strip compact and vertically centered', async () => {
    ...
    expect(stats.get('.session-platform-stats').classes()).toContain(
  ...
  it('keeps batch revoke in the table action toolbar', async () => {
    ...
    expect(
      wrapper.find('.app-table__toolbar-left [data-testid="session-batch-revoke"]').exists(),
    ).toBe(true)
  })
  ...
  it('renders explicit loading, empty, and error states', async () => {
    let resolveList:
      | ((value: { list: SessionItem[]; total: number; page: number; pageSize: number }) => void)
      | undefined
    ...
    const failed = mountPage(['user:session:list'])
    await flushPromises()
  ...
  return [
    {
      ...
      refreshExpiresAt: '2026-08-22T00:00:00Z',
      revokedAt: null,
      status: 'active',
      isCurrent: true,
    },
