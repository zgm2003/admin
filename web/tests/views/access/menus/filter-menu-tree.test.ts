...
const tree = (): ManagedMenuNode[] => [{ id: 1, parentId: null, menuType: 'directory', name: '权限与认证', code: 'access', i18nKey: 'navigation.access', path: null, componentPath: null, icon: 'lucide:shield-check', sortOrder: 10, isEnabled: 1, isHidden: 0, isProtected: 0, createdAt: '2026-08-19T02:00:00Z', updatedAt: '2026-08-19T02:00:00Z', children: [{ id: 2, parentId: 1, menuType: 'page', name: '菜单管理', code: 'rbac:menu:list', i18nKey: 'navigation.accessMenus', path: '/access/menus', componentPath: 'access/menus', icon: 'lucide:panel-left', sortOrder: 10, isEnabled: 1, isHidden: 0, isProtected: 1, createdAt: '2026-08-19T02:00:00Z', updatedAt: '2026-08-19T02:00:00Z', children: [action(3, 'rbac:menu:create', '新增菜单'), action(4, 'rbac:menu:update', '修改菜单')] }] }]

describe('filterManagedMenuTree', () => {
  it('keeps matches, ancestors, and matched-node descendants without mutating input', () => {
    const source = tree()
    const result = filterManagedMenuTree(source, 'rbac:menu:create')
    expect(result.map((node) => node.code)).toEqual(['access'])
    expect(result[0]?.children.map((node) => node.code)).toEqual(['rbac:menu:list'])
    expect(result[0]?.children[0]?.children.map((node) => node.code)).toEqual(['rbac:menu:create'])
