...
import {
  ...
  updateRoleStatus,
} from '@src/api/rbac/role'
import type { RoleListItem } from '@src/api/rbac/role'
import { YesNo } from '@src/enums/yes-no'
import { appI18n, setLocale } from '@src/i18n'
import { useAccessStore } from '@src/store/access'
import RolePermissionMatrix from '@src/modules/rbac/role/components/RolePermissionMatrix.vue'
import RoleManagement from '@src/modules/rbac/role/index.vue'

vi.mock('@src/api/rbac/role', () => ({
  getRoles: vi.fn(),
  createRole: vi.fn(),
  updateRole: vi.fn(),
...
describe('RoleManagement', () => {
  ...
  it('loads explicit pagination once and renders the role row', async () => {
    const wrapper = mountPage(['rbac:role:list'])
    await flushPromises()
  ...
  it('renders commands only for their exact permissions', async () => {
    const wrapper = mountPage(['rbac:role:create'])
    await flushPromises()
  ...
  it.each([
    ['rbac:role:create', '新增角色'],
    ['rbac:role:update', '编辑'],
    ['rbac:role:status', '禁用'],
    ['rbac:role:default', '设为默认'],
    ['rbac:role:delete', '删除'],
    ['rbac:role:authorize', '授权'],
  ...
  it('keeps code readonly and preserves filters while editing the role name', async () => {
    const wrapper = mountPage(['rbac:role:update'])
    await flushPromises()
    await wrapper.get('.role-filters input').setValue('tester')
    await findButton(wrapper, '查询').trigger('click')
    await flushPromises()
  ...
  it('soft deletes after confirmation and moves back only when the current page becomes invalid', async () => {
    ...
    const wrapper = mountPage(['rbac:role:delete'])
    await flushPromises()
    wrapper.getComponent(ElPagination).vm.$emit('current-change', 2)
    await flushPromises()
  ...
  it('shows the effective permission diff before submitting minimal direct grants', async () => {
    ...
    expect(document.body.textContent).toContain('新增角色 · rbac:role:create')
    ...
    expect(accessStore.permissionCodes).toEqual(['rbac:role:authorize'])
