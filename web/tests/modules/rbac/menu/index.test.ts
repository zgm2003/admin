...
import MenuManagement from '@src/modules/rbac/menu/index.vue'

vi.mock('@src/api/rbac/menu', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@src/api/rbac/menu')>()
  return {
    ...actual,
    getMenus: vi.fn(),
    createMenu: vi.fn(),
...
describe('MenuManagement', () => {
  ...
  it('defines menu type labels as a computed locale-dependent value', () => {
    const source = readFileSync(resolve(process.cwd(), 'src/modules/rbac/menu/index.vue'), 'utf8')
  ...
  it('lets the menu dialog size itself to the form content', async () => {
    ...
    await flushPromises()
		...
		expect(document.body.textContent).toContain('页面路径：不能以 / 开头，页面文件为 web/src/modules/<页面路径>/index.vue')
