...
export interface MenuPlatformOption { id:number; code:string; name:string; isEnabled:YesNo }
export interface ManagedMenuNode { id:number; platformId:number; platformCode:string; platformName:string; parentId:number|null; menuType:ManagedMenuType; name:string; code:string; i18nKey:string|null; path:string|null; componentPath:string|null; icon:MenuIconName|null; sortOrder:number; isEnabled:YesNo; isHidden:YesNo; isProtected:YesNo; createdAt:string; updatedAt:string; children:ManagedMenuNode[] }
export interface MenuCatalogResponse { platforms:MenuPlatformOption[]; menuTree:ManagedMenuNode[] }
export interface MenuListQuery { platformId:number }
export interface CreateMenuInput { platformId:number; parentId:number|null; menuType:ManagedMenuType; name:string; code:string; i18nKey:string|null; path:string|null; componentPath:string|null; icon:MenuIconName|null; sortOrder:number; isEnabled:YesNo; isHidden:YesNo }
...
export async function getMenus(query?: MenuListQuery): Promise<MenuCatalogResponse> {
  ...
  const raw = query === undefined
    ...
    : await request<unknown>({ method: 'GET', url: '/api/admin/v1/menus', params: { platformId: query.platformId } })
...
const menuNodeKeys = [
  'id', 'platformId', 'platformCode', 'platformName', 'parentId', 'menuType', 'name', 'code',
  'i18nKey', 'path', 'componentPath', 'icon', 'sortOrder', 'isEnabled', 'isHidden', 'isProtected',
  'createdAt', 'updatedAt', 'children',
] as const

function parseMenuCatalog(value: unknown, requestedPlatformID?: number): MenuCatalogResponse {
  if (!hasExactKeys(value, ['platforms', 'menuTree']) || !Array.isArray(value.platforms) || !Array.isArray(value.menuTree)) {
  ...
  if (requestedPlatformID !== undefined && !platformsByID.has(requestedPlatformID)) throw invalidMenuCatalog()
  const menuTree = value.menuTree.map((node) => parseManagedMenuNode(node, platformsByID, null, requestedPlatformID))
  return { platforms, menuTree }
}

function parseMenuPlatform(value: unknown): MenuPlatformOption {
  if (!hasExactKeys(value, platformKeys) || !isPositiveInteger(value.id) || !isNonEmptyString(value.code) ||
    !isNonEmptyString(value.name) || !isYesNo(value.isEnabled)) {
    throw invalidMenuCatalog()
  }
...
function parseManagedMenuNode(
  ...
  if (platform === undefined || platform.code !== value.platformCode || platform.name !== value.platformName ||
  ...
  return {
    ...
    children: value.children.map((child) => parseManagedMenuNode(child, platformsByID, value.id, requestedPlatformID)),
  }
}
...
function isManagedMenuNodeRecord(value: unknown): value is ManagedMenuNodeRecord {
  return hasExactKeys(value, menuNodeKeys) && isPositiveInteger(value.id) && isPositiveInteger(value.platformId) &&
    isNonEmptyString(value.platformCode) && isNonEmptyString(value.platformName) &&
    isNullablePositiveInteger(value.parentId) && isManagedMenuType(value.menuType) && isNonEmptyString(value.name) &&
    isNonEmptyString(value.code) && isNullableString(value.i18nKey) && isNullableString(value.path) &&
    isNullableString(value.componentPath) && isNullableMenuIcon(value.icon) &&
    isNonNegativeInteger(value.sortOrder) && isYesNo(value.isEnabled) && isYesNo(value.isHidden) &&
    isYesNo(value.isProtected) && isNonEmptyString(value.createdAt) && isNonEmptyString(value.updatedAt) &&
