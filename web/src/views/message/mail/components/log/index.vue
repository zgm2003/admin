...
import { deleteMailLog, deleteMailLogs, getMailLogDetail, type MailLog, type MailLogDetail } from '../../../../../api/system/mail'
...
const pagination = computed<TablePaginationState>(() => ({ currentPage: props.page, pageSize: props.pageSize, total: props.total }))
  ...
  <div class="table-tab">
    ...
    >
      <template #toolbar-left>
        ...
        <el-button v-if="canDelete" data-testid="mail-log-batch-delete" text type="danger" :disabled="selectedCount === 0" @click="removeSelected">
          <Trash2 :size="16" />
          {{ t('mail.batchDelete') }}
        </el-button>
      </template>
      <template #cell-status="{ row }: { row: MailLog }">
        <el-tag :type="row.status === 'sent' ? 'success' : row.status === 'failed' ? 'danger' : 'warning'" effect="plain">{{ row.status }}</el-tag>
      </template>
    ...
    <el-drawer v-model="detailVisible" :title="t('mail.logDetail')" size="min(480px, 94vw)">
      <el-descriptions v-if="detail" :column="1" border>
        <el-descriptions-item :label="t('mail.recipient')">{{ detail.log.toEmail }}</el-descriptions-item>
        <el-descriptions-item :label="t('mail.scene')">{{ detail.log.scene }}</el-descriptions-item>
        <el-descriptions-item :label="t('mail.status')">{{ detail.log.status }}</el-descriptions-item>
        <el-descriptions-item label="Request ID"><code>{{ detail.log.requestId || '-' }}</code></el-descriptions-item>
        <el-descriptions-item label="Message ID"><code>{{ detail.log.messageId || '-' }}</code></el-descriptions-item>
        <el-descriptions-item :label="t('mail.verificationCode')"><strong>{{ detail.verificationCode || '-' }}</strong></el-descriptions-item>
        <el-descriptions-item :label="t('mail.expiresAt')">{{ detail.verificationExpiresAt || '-' }}</el-descriptions-item>
        <el-descriptions-item :label="t('mail.error')">{{ detail.log.errorSummary || '-' }}</el-descriptions-item>
      </el-descriptions>
    </el-drawer>
  </div>
</template>
