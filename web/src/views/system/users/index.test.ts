...
vi.mock('../../../api/user', () => ({ getUsers:vi.fn(), getUserRoleOptions:vi.fn(), updateUser:vi.fn(), updateUserStatus:vi.fn(), deleteUser:vi.fn(), getUserRoles:vi.fn(), updateUserRoles:vi.fn() }))
...
describe('user management', () => {
  ...
  it('renders only granted commands and protects self and super targets', async () => {
    ...
    expect(texts.join(' ')).toContain('分配角色')
		const dangerous = wrapper.findAll('button').filter((button) => ['修改用户状态','删除用户','分配角色'].includes(button.attributes('aria-label') ?? ''))
  ...
  it('confirms status and delete consequences then refreshes the applied page', async () => {
    ...
    await flushPromises()
		await findAriaButton(wrapper, '修改用户状态').trigger('click'); await flushPromises()
...
function findAriaButton(wrapper: VueWrapper, label: string) { return wrapper.get(`button[aria-label="${label}"]`) }
