...
import type { UserListItem, UserListQuery, UserRolesResponse, UserRoleSummary } from '../../../api/user.contract'
...
const canDelete = computed(() => access.hasPermission('system:user:delete'))
const canRoles = computed(() => access.hasPermission('system:user:roles'))
...
const usernameValid = computed(() => {
  ...
  return characters.length >= 3 && characters.length <= 64 && characters.every((character) => /[\p{L}\p{N}_-]/u.test(character))
})
...
async function loadRoleOptions(): Promise<void> {
  ...
  roleOptionsLoading.value = true; roleOptionsError.value = ''
  try { roleOptions.value = (await getUserRoleOptions()).roles }
  catch (error: unknown) { roleOptionsError.value = errorMessage(error, 'user.roleLoadFailed') }
  finally { roleOptionsLoading.value = false }
}
function search(): void {
  const value = keyword.value.trim()
  query.value = { page:1, pageSize:query.value.pageSize, ...(value === '' ? {} : {keyword:value}), ...(statusFilter.value === '' ? {} : {isEnabled:statusFilter.value}), ...(roleFilter.value === '' ? {} : {roleId:roleFilter.value}) }
  void loadUsers()
}
...
function editDisabled(row:UserListItem):boolean { return targetProtected(row) }
function dangerDisabled(row:UserListItem):boolean { return isSelf(row) || targetProtected(row) }
function protectionText(row:UserListItem, operation:'status'|'roles'|'delete'):string {
  ...
  if (isSelf(row)) return t(operation==='status'?'user.selfStatusBlocked':operation==='roles'?'user.selfRolesBlocked':'user.selfDeleteBlocked')
  return operation==='roles' ? t('user.assignRoles') : operation==='delete' ? t('permission.userDelete') : t('user.status')
}

function openEdit(row:UserListItem):void { if(editDisabled(row)) return; editingUser.value=row; userForm.value={username:row.username}; editError.value=''; editVisible.value=true }
async function saveEdit():Promise<void> {
  ...
  try {
    const result=await updateUser(target.id,{username:normalizedUsername.value})
    auth.updateUsername(result.id,result.username)
    if(await loadUsers()){ editVisible.value=false; ElNotification.success({title:t('user.updateSuccess')}) }
  } catch(error:unknown){ editError.value=errorMessage(error,'user.saveFailed') }
  finally{ editSaving.value=false }
}

async function openRoles(row:UserListItem):Promise<void> {
  if(dangerDisabled(row)) return
  roleTarget.value=row; roleDialogVisible.value=true; roleLoading.value=true; roleError.value=''; roleData.value=null; selectedRoleIDs.value=[]
  try { const data=await getUserRoles(row.id); roleData.value=data; selectedRoleIDs.value=[...data.roleIds] }
  catch(error:unknown){ roleError.value=errorMessage(error,'user.roleLoadFailed') }
...
function clearRoles():void { selectedRoleIDs.value=protectedSelectedRoleIDs() }
function roleToggleDisabled(role:UserRoleSummary):boolean { return role.code==='super_admin'&&!isSuperAdminActor.value }
async function saveRoles():Promise<void> {
  const target=roleTarget.value
  if(target===null||roleData.value===null||!hasEnabledSelection.value||roleSaving.value)return
  ...
  try {
    ...
    if(await loadUsers()){ roleDialogVisible.value=false; ElNotification.success({title:t('user.rolesSuccess')}) }
  } catch(error:unknown){ roleError.value=errorMessage(error,'user.saveFailed') }
...
async function changeStatus(row:UserListItem):Promise<void> {
  ...
  try { await ElMessageBox.confirm(message,t('user.status'),{type:'warning'}); mutating.value=true; await updateUserStatus(row.id,next); await loadUsers(); ElNotification.success({title:t('user.statusSuccess')}) }
  catch(error:unknown){ if(error!=='cancel'&&error!=='close')mutationError.value=errorMessage(error,'user.saveFailed') }
  finally{ mutating.value=false }
}
async function removeUser(row:UserListItem):Promise<void> {
  if(dangerDisabled(row)||mutating.value)return
  let message=t('user.deleteConfirm'); if(hasSuperAdmin(row))message+=` ${t('user.superAdminImpact')}`
  try {
    await ElMessageBox.confirm(message,t('permission.userDelete'),{type:'warning'}); mutating.value=true; await deleteUser(row.id)
    const maxPage=Math.max(1,Math.ceil((total.value-1)/query.value.pageSize)); if(query.value.page>maxPage)query.value={...query.value,page:maxPage}
    await loadUsers(); ElNotification.success({title:t('user.deleteSuccess')})
  } catch(error:unknown){ if(error!=='cancel'&&error!=='close')mutationError.value=errorMessage(error,'user.saveFailed') }
  finally{ mutating.value=false }
}

onMounted(()=>{ void loadRoleOptions(); void loadUsers() })
</script>
