  ...
  set: (value) => {
    ...
    timeRange.value =
      ...
      value.timeRange.every((item) => typeof item === 'string')
        ? [value.timeRange[0] as string, value.timeRange[1] as string]
        : []
  },
})
...
const tableColumns = computed<TableColumn<OperationLogItem>[]>(() => [
  ...
  { prop: 'route', label: t('operationLog.column.route'), minWidth: 220, overflowTooltip: true },
  {
    key: 'user',
    prop: 'id',
    label: t('operationLog.column.user'),
    minWidth: 150,
...
<template>
  <section class="operation-log-page management-page">
    ...
    >
      <template #expand="{ row }: { row: OperationLogItem }">
        <div class="operation-log-detail">
          ...
          <el-row :gutter="12" class="operation-log-detail__meta">
            <el-col :xs="24" :sm="8"
              ><dt>{{ t('operationLog.requestId') }}</dt>
              <dd>
                <code>{{ row.requestId }}</code>
              </dd></el-col
            >
            <el-col :xs="24" :sm="8"
              ><dt>{{ t('operationLog.platform') }}</dt>
              <dd>{{ row.platform }}</dd></el-col
            >
            <el-col :xs="24" :sm="8"
              ><dt>{{ t('operationLog.userAgent') }}</dt>
              <dd>{{ row.userAgent }}</dd></el-col
            >
          </el-row>
          <el-row :gutter="16" class="operation-log-summaries">
            <el-col :xs="24" :sm="12"
              ><section>
                <h3>{{ t('operationLog.detail.request') }}</h3>
                <pre>{{ formatJSON(row.requestData) }}</pre>
              </section></el-col
            >
            <el-col :xs="24" :sm="12"
              ><section>
                <h3>{{ t('operationLog.detail.response') }}</h3>
                <pre>{{ formatJSON(row.responseData) }}</pre>
              </section></el-col
            >
          </el-row>
        </div>
      </template>
      <template #cell-method="{ row }: { row: OperationLogItem }"
      ...
      <template #cell-status="{ row }: { row: OperationLogItem }"
        ><el-tag :type="row.isSuccess === YesNo.Yes ? 'success' : 'danger'" effect="light">{{
          row.statusCode
        }}</el-tag></template
      >
      <template #cell-latency="{ row }: { row: OperationLogItem }">{{ row.latencyMs }} ms</template>
      <template #cell-createdAt="{ row }: { row: OperationLogItem }">{{
