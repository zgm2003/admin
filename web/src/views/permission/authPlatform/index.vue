...
const canUpdate = computed(() => access.hasPermission('permission:authPlatform:update'))
...
async function submit(): Promise<void> {
  ...
  if (dialogMode.value === 'edit' && editingPlatform.value !== null) {
    if (
      ...
      !(await confirmAction('permission.authPlatform.confirm.limit'))
    )
      return
    if (
      authPlatformSecurityChanged(form.value, editingPlatform.value) &&
      !(await confirmAction('permission.authPlatform.confirm.security'))
    )
      return
  }
  ...
  try {
    ...
    ElNotification.success({
      title: t(
        dialogMode.value === 'create'
          ? 'permission.authPlatform.success.created'
          : 'permission.authPlatform.success.updated',
      ),
    })
  } catch (error: unknown) {
    mutationError.value = errorMessage(error, 'permission.authPlatform.mutationFailed')
  } finally {
...
async function toggleStatus(platform: AuthPlatformListItem): Promise<void> {
  ...
  try {
    ...
    await loadPage()
    ElNotification.success({ title: t('permission.authPlatform.success.status') })
  } catch (error: unknown) {
    mutationError.value = errorMessage(error, 'permission.authPlatform.mutationFailed')
  }
}
...
async function remove(platform: AuthPlatformListItem): Promise<void> {
  ...
  try {
    ...
    await loadPage()
    ElNotification.success({ title: t('permission.authPlatform.success.deleted') })
  } catch (error: unknown) {
    mutationError.value = errorMessage(error, 'permission.authPlatform.mutationFailed')
  }
}
...
async function confirmAction(
  key:
    ...
    | 'permission.authPlatform.confirm.delete'
    | 'permission.authPlatform.confirm.limit'
    | 'permission.authPlatform.confirm.security',
): Promise<boolean> {
  try {
    await ElMessageBox.confirm(t(key), t('permission.authPlatform.title'), {
      type: 'warning',
    })
    return true
  } catch (error: unknown) {
...
function errorMessage(
  error: unknown,
  fallbackKey: 'permission.authPlatform.loadFailed' | 'permission.authPlatform.mutationFailed',
): string {
...
<template>
  <section class="auth-platform-page management-page">
    <AppSearch
      ...
      :fields="searchFields"
      :query-label="t('permission.authPlatform.search')"
      :reset-label="t('permission.authPlatform.reset')"
    ...
    <AppTable
      ...
      :aria-label="t('permission.authPlatform.title')"
      :refresh-label="t('permission.authPlatform.refresh')"
      @refresh="refresh"
    ...
    >
      <template #toolbar-left>
        <el-button
          ...
          @click="openCreate"
          >{{ t('permission.authPlatform.create') }}</el-button
        >
      </template>
      ...
      <template #cell-tokenTTL="{ row }: { row: AuthPlatformListItem }">
        <div class="auth-platform-policy-stack">
          <div>
            <span>{{ t('permission.authPlatform.accessToken') }}</span
            ><strong
              :title="t('permission.authPlatform.seconds', { count: row.accessTTLSeconds })"
              >{{ ttlLabel(row.accessTTLSeconds) }}</strong
            >
          </div>
          <div>
            <span>{{ t('permission.authPlatform.refreshToken') }}</span
            ><strong
              :title="t('permission.authPlatform.seconds', { count: row.refreshTTLSeconds })"
              >{{ ttlLabel(row.refreshTTLSeconds) }}</strong
            >
          </div>
        </div>
      </template>
      <template #cell-cacheTTL="{ row }: { row: AuthPlatformListItem }">
        <div class="auth-platform-policy-stack">
          <div>
            <span>{{ t('permission.authPlatform.sessionCache') }}</span
            ><strong
              :title="t('permission.authPlatform.seconds', { count: row.sessionCacheTTLSeconds })"
              >{{ ttlLabel(row.sessionCacheTTLSeconds) }}</strong
            >
          </div>
          <div>
            <span>{{ t('permission.authPlatform.accessCache') }}</span
            ><strong
              :title="t('permission.authPlatform.seconds', { count: row.accessCacheTTLSeconds })"
              >{{ ttlLabel(row.accessCacheTTLSeconds) }}</strong
            >
          </div>
        </div>
      </template>
            ...
            >{{
              t(
                row.bindDevice === YesNo.Yes
                  ? 'permission.authPlatform.deviceBound'
                  : 'permission.authPlatform.deviceUnbound',
              )
            }}</el-tag
          >
          <el-tag
            ...
            >{{
              t(
                row.bindIP === YesNo.Yes
                  ? 'permission.authPlatform.ipBound'
                  : 'permission.authPlatform.ipUnbound',
              )
            }}</el-tag
          >
        </div>
          ...
          >{{
            t(
              row.allowRegister === YesNo.Yes
                ? 'permission.authPlatform.registrationAllowed'
                : 'permission.authPlatform.registrationDenied',
            )
          }}</el-tag
        >
      </template>
        ...
        ><el-tag size="small" :type="row.isEnabled === YesNo.Yes ? 'success' : 'danger'">{{
          t(
            row.isEnabled === YesNo.Yes
              ? 'permission.authPlatform.enabled'
              : 'permission.authPlatform.disabled',
          )
        }}</el-tag></template
      >
        ...
        ><template v-if="row.id > 0">
          ...
          <el-button
            ...
            >{{
              t(
                row.isEnabled === YesNo.Yes
                  ? 'permission.authPlatform.disable'
                  : 'permission.authPlatform.enable',
              )
            }}</el-button
          >
          <el-button
            ...
            @click="remove(row)"
            >{{ t('permission.authPlatform.delete') }}</el-button
          >
        </template></template
      >
      <template #empty><el-empty :description="t('permission.authPlatform.empty')" /></template>
    </AppTable>
