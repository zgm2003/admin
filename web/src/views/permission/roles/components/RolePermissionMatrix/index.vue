...
const groupRuntimeStateMap = computed(() => {
  ...
  for (const group of props.groups) {
    ...
    const actionIDs = group.rows.flatMap((row) => row.actions.map((action) => action.id))
    states.set(group.groupKey, {
      selection: getRoleMatrixSelectionState(getRoleMatrixGroupMenuIDs(group), selected),
...
function selectedActionCount(row: RoleMatrixRow): number {
  return row.actions.filter((action) => selectedMenuIDSet.value.has(action.id)).length
}
...
function setActionChecked(row: RoleMatrixRow, actionID: number, checked: boolean): void {
  selectedMenuIDs.value = toggleMatrixAction(selectedMenuIDs.value, row, actionID, checked)
}

function setGroupChecked(group: RoleMatrixGroup, checked: boolean): void {
  selectedMenuIDs.value = toggleMatrixGroup(selectedMenuIDs.value, group, checked)
}
...
<template>
  <el-empty v-if="groups.length === 0" :description="t('role.permission.empty')" />
  <div v-else class="role-permission-matrix">
    <section v-for="group in groups" :key="group.groupKey" class="role-permission-matrix__group">
      <div class="role-permission-matrix__group-header">
        <div class="role-permission-matrix__group-main">
          ...
          >
            ...
            <el-tag v-if="group.groupIsEnabled === YesNo.No" size="small" type="danger">
          ...
          <div class="role-permission-matrix__group-meta">
            ...
            <span>
              ...
              {{ groupRuntimeState(group).pageSelected }}/{{ groupRuntimeState(group).pageTotal }}
            </span>
            <span>
        ...
        <el-space>
          <el-button size="small" text @click="toggleGroupCollapse(group)">
            {{ t(isGroupCollapsed(group) ? 'role.permission.expand' : 'role.permission.collapse') }}
          </el-button>
          <el-button size="small" text type="primary" @click="setGroupChecked(group, true)">
      ...
      <div v-if="!isGroupCollapsed(group)" class="role-permission-matrix__table-scroll">
        ...
        <el-table :data="group.rows" border row-key="pageId" class="role-permission-matrix__table">
            ...
            <template #default="{ row }">
              ...
              >
                <span class="role-permission-matrix__permission-copy">
                  ...
                  <el-tag v-if="row.pageIsEnabled === YesNo.No" size="small" type="danger">
                    {{ t('role.permission.disabled') }}
                  </el-tag>
                </span>
              </el-checkbox>
          ...
          <el-table-column :label="t('role.permission.action')" min-width="460">
            <template #default="{ row }">
              <el-space v-if="row.actions.length > 0" wrap class="role-permission-matrix__actions">
                <el-checkbox
                  ...
                  @update:model-value="
                    (value: unknown) => setActionChecked(row, action.id, Boolean(value))
                  "
                >
                  <span class="role-permission-matrix__permission-copy">
                    <strong>{{ action.name }}</strong>
                    <span>{{ action.code }}</span>
                    <el-tag v-if="action.isEnabled === YesNo.No" size="small" type="danger">
                      {{ t('role.permission.disabled') }}
                    </el-tag>
                  </span>
                </el-checkbox>
